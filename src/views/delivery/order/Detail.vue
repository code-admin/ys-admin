<template>
  <div class="page">
    ...
    <div class="card flex-sa">
      ...
      <el-button size="mini" icon="el-icon-back" @click="$router.back()">返回</el-button>
    </div>

    <div class="card mt20">
      <el-steps
        :active="orderDeliver.status"
      ...
      <el-row :gutter="20">
        ...
        <el-col :xs="24" :sm="12" :md="8" :lg="6" :xl="4">
          <el-row :gutter="10">
            ...
            <el-col :span="16">
              <div class="val">
                <el-button type="text" size="mini">{{ orderDeliver.shippingAddress ? orderDeliver.shippingAddress.address : '--' }}</el-button>
              </div>
            </el-col>
          </el-row>
        </el-col>
    ...
    <div class="card mt20">
      ...
      <el-card v-for="order in orderDeliver.orderList" :key="order.id" class="box-card mb20">
        <div slot="header" class="clearfix">
          <span class="title">订单号: XS202111112670</span>

          <el-button size="mini" icon="el-icon-printer" @click="extractedGoods(order.id)">打印提货单</el-button>
          <el-button type="primary" size="mini" icon="el-icon-truck" @click="batchInit(order)">批量出库</el-button>
        </div>
        <el-row :gutter="20">
          <el-col :xs="24" :sm="12" :md="8" :lg="6" :xl="4">
            <el-row :gutter="10">
        ...
        <el-table :data="order.orderExts" style="width: 100%" size="mini">
          <el-table-column prop="requirement" label="筒布" align="center" show-overflow-tooltip>
          ...
          <el-table-column v-if="order.orderType === 1" prop="goodsLength" label="长度(cm)" align="center" />
          <el-table-column v-if="order.orderType === 1" prop="number" label="条数" align="center" />
    ...
    <div class="card mt20">
      ...
      <el-table :data="orderDeliver.orderExpressList" style="width: 100%" size="mini" @selection-change="changePrint">
        <el-table-column prop="orderId" label="销售订单编号" align="center" width="140">
          <template slot-scope="{row}">
            <el-button type="text" size="mini" @click="orderDetail(row)">{{ row.orderId }}</el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="width" label="宽度(CM)" align="center" />
        <el-table-column prop="weight" label="克重(G)" align="center" />
        <el-table-column v-if="orderDeliver.orderType === 1" prop="goodsLength" label="长度(cm)" align="center" />
        <el-table-column v-if="orderDeliver.orderType === 1" prop="productNumber" label="条数" align="center" />
    ...
    <el-dialog :close-on-click-modal="false" :title="`添加 ( ${order.orderNo} ) 订单出库`" :visible.sync="outStockVisible" width="80%">
      ...
      <el-table :data="outStockList" size="mini">
        ...
        <el-table-column v-if="order.orderType === 1" prop="goodsLength" label="长度" align="center" width="160" />
        <el-table-column v-if="order.orderType === 1" prop="productNumber" label="出库条数" align="center" width="160">
          <template slot-scope="scope">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" icon="el-icon-finished" :loading="submitOutstockLoading" @click="saveExpress">保存并出库</el-button>
      </div>
    </el-dialog>
...
import { detail, submitExpressOrder } from '@/api/delivery.js'
