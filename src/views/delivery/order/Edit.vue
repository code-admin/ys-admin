<template>
  <div class="page">
        ...
        >
          <template slot-scope="scope">
            <el-button
              type="text"
              size="mini"
              @click="showOutStock(scope.row, true)"
            >出库</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <div class="card mt20">
      ...
      <el-card v-for="(order,index) in orderInfo.orderList" :key="index" class="box-card mb20">
        <div slot="header" class="clearfix">
          <span class="title">订单号: {{ order.orderNo }}</span>
          <el-button style="float: right; padding: 3px 0" type="text" @click="deleteSelectByIndex(index)">删除</el-button>
        </div>
        <el-row :gutter="20">
      ...
      <el-button round size="medium" icon="el-icon-plus" plain style="width:100%" @click="getConsumerOrder">加入销售订单</el-button>
    </div>

    <div class="card mt20 flex-end">
      <el-button icon="el-icon-back" size="mini" @click="$router.back()">返回</el-button>
      <el-button type="primary" icon="el-icon-edit" size="mini" @click="saveData">保存</el-button>
    </div>

    <!-- 弹框选择订单 -->
    <el-dialog title="请选择需要配送的订单" :visible.sync="dialogVisible" width="90%" :before-close="handleClose">
      <el-table key="2" :data="consumerOrderList" size="mini" @selection-change="selectTable">
        <el-table-column
          type="selection"
          width="55"
        />
...
export default {
  ...
  methods: {
    ...
    // 获取待配送的订单数据
    getConsumerOrder() {
      const ags = { status: 2, makingType: 1, loadingExtFlag: true, pageIndex: 1, pageSize: 100 }
